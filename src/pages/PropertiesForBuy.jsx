...
const PropertyCardApp = ({ item, index, navigate }) => {
  ...
  const carouselId = `carousel-buy-${item.id}`;
        ...
        <div className="position-relative media-frame overflow-hidden">
          ...
          <div className="position-absolute top-0 start-0 m-2 z-3">
            <span className="badge badge-subtle-tag px-2-5 py-1-5 rounded-pill shadow-sm d-flex align-items-center gap-1">
              <Sparkles size={11} className="text-warning" /> FOR SALE
            </span>
          </div>
  ...
  return (
    <div className="py-5" style={{ backgroundColor: '#f8fafc', minHeight: '100vh' }}>
      <div className="container py-3">
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center my-3 pb-3 border-bottom gap-3" data-aos="fade-down">
          <div>
            <span
            ...
            >
              <Sparkles size={12} className="me-1" /> VERIFIED INVENTORY
            </span>
            <h2 className="fw-extrabold m-0 text-dark display-6">
              Premium Properties <span style={{ background: 'linear-gradient(135deg, #ef4444, #0284c7)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>For Sale</span>
            </h2>
          </div>
          <div>
            <span className="badge rounded-pill bg-white text-dark px-4 py-2 border shadow-sm fw-bold">
        ...
        {properties.length === 0 ? (
          <div className="text-center py-5 bg-white rounded-4 shadow-sm my-4">
            <h5 className="fw-bold text-dark m-0">No purchase listings found.</h5>
          </div>
        ) : (
